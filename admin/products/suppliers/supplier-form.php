<?php
require_once($_SERVER['DOCUMENT_ROOT'] . '/carriemart/includes/admin-auth.php');
require_once($_SERVER['DOCUMENT_ROOT'] . '/carriemart/includes/config.php');

if (!$conn) {
    die('Database connection failed.');
}

// Inputs
$id_raw = isset($_GET['id']) ? trim($_GET['id']) : '';
$supplier_id = ctype_digit($id_raw) ? (int)$id_raw : 0;
$isEdit = $supplier_id > 0;

// Defaults
$supplier = [
    'supplier_id'    => '',
    'supplier_name'  => '',
    'contact_person' => '',
    'contact_number' => '',
    'email'          => '',
    'address'        => '',
    'is_active'      => 1,
    'created_at'     => ''
];

// Load existing
if ($isEdit) {
    $stmt = $conn->prepare("SELECT supplier_id, supplier_name, contact_person, contact_number, email, address, is_active, created_at FROM suppliers WHERE supplier_id = ?");
    if (!$stmt) {
        header('Location: index.php?error=server');
        exit;
    }
    $stmt->bind_param('i', $supplier_id);
    $stmt->execute();
    $stmt->bind_result(
        $supplier['supplier_id'],
        $supplier['supplier_name'],
        $supplier['contact_person'],
        $supplier['contact_number'],
        $supplier['email'],
        $supplier['address'],
        $supplier['is_active'],
        $supplier['created_at']
    );
    if (!$stmt->fetch()) {
        $stmt->close();
        header('Location: index.php?error=not_found');
        exit;
    }
    $stmt->close();
}

// Determine action
$formAction = $isEdit ? 'update.php' : 'create.php';

// Error codes mapping (from create/update pages)
$errors = [];
if (isset($_GET['error'])) {
    $codes = explode(',', $_GET['error']);
    foreach ($codes as $e) {
        $e = trim($e);
        if ($e === '') continue;
        if ($e === 'invalid_id')     $errors[] = 'Invalid supplier ID.';
        if ($e === 'not_found')      $errors[] = 'Supplier not found.';
        if ($e === 'name_required')  $errors[] = 'Supplier name is required.';
        if ($e === 'status_invalid') $errors[] = 'Status value is invalid.';
        if ($e === 'email_invalid')  $errors[] = 'Email address is invalid.';
        if ($e === 'duplicate')      $errors[] = 'A supplier with the same name already exists.';
        if ($e === 'server')         $errors[] = 'Server error. Please try again.';
    }
}

// Success status
$success = false;
$successText = '';
if (isset($_GET['status'])) {
    if ($_GET['status'] === 'created') {
        $success = true;
        $successText = 'Supplier created.';
    } elseif ($_GET['status'] === 'updated') {
        $success = true;
        $successText = 'Supplier updated.';
    }
}

// Helper
$sel = function($a, $b){ return ((string)$a === (string)$b) ? 'selected' : ''; };
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>CarrieMart: Supplier Form</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet"
          integrity="sha384-sRIl4kxILFvY47J16cr9ZwB07vP4J8+LH7qKQnuqkuIAvNWLzeN8tE5YBujZqJLB" crossorigin="anonymous">
    <style>
        .form-register { background-color:#ffffff; border-radius:1rem; padding:1rem; }
        .btn-icon-inverted img { width:1.125rem; height:1.125rem; filter:invert(43%) sepia(6%) saturate(179%) hue-rotate(169deg) brightness(92%) contrast(88%); opacity:.95; }
        .btn-icon img { width:1.125rem; height:1.125rem; filter:brightness(0) invert(1); }
        .avatar-lg { width:96px; height:96px; border-radius:50%; object-fit:cover; background:#f1f3f5; }
        .label-small { font-size:.8rem; color:var(--bs-secondary-color); }
    </style>
</head>

<body>
<div class="container">
    <main class="form-register">
        <div class="py-4 text-center">
            <img class="d-block mx-auto mb-0" src="/carriemart/assets/Logo.svg" alt="" width="72" height="57">
        </div>

        <div class="row g-5">
            <div class="col-md-8 col-lg-7 mx-auto">
                <h4 class="mb-3"><?php echo $isEdit ? 'Edit Supplier' : 'Add Supplier'; ?></h4>

                <?php if (!empty($errors)): ?>
                    <div class="alert alert-danger mb-0" role="alert">
                        <?php foreach ($errors as $er): ?>
                            <div>- <?php echo $er; ?></div>
                        <?php endforeach; ?>
                    </div class="mb-2">
                <?php elseif ($success): ?>
                    <div class="alert alert-success d-flex justify-content-between align-items-center mb-0" role="alert">
                        <span><?php echo $successText; ?></span>
                        <a class="btn btn-success" href="/carriemart/admin/products/suppliers/index.php" role="button">OK</a>
                    </div>
                <?php endif; ?>

                <form method="post" action="<?php echo $formAction; ?>">
                    <div class="row g-3">
                        <div class="col-md-4">
                            <label class="form-label">Supplier ID</label>
                            <input type="text" class="form-control" value="<?php echo $supplier['supplier_id']; ?>" disabled>
                            <?php if ($isEdit): ?>
                                <input type="hidden" name="supplier_id" value="<?php echo $supplier['supplier_id']; ?>">
                            <?php endif; ?>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Created at</label>
                            <input type="text" class="form-control" value="<?php echo $supplier['created_at']; ?>" disabled>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Status</label>
                            <select name="is_active" class="form-select">
                                <option value="1" <?php echo $sel($supplier['is_active'], 1); ?>>active</option>
                                <option value="0" <?php echo $sel($supplier['is_active'], 0); ?>>inactive</option>
                            </select>
                        </div>
                    </div>

                    <hr class="my-4">

                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Supplier name</label>
                            <input type="text" name="supplier_name" class="form-control" value="<?php echo $supplier['supplier_name']; ?>">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Contact person</label>
                            <input type="text" name="contact_person" class="form-control" value="<?php echo $supplier['contact_person']; ?>">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Contact number</label>
                            <input type="text" name="contact_number" class="form-control" placeholder="+63" value="<?php echo $supplier['contact_number']; ?>">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Email</label>
                            <input type="text" name="email" class="form-control" value="<?php echo $supplier['email']; ?>">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Address</label>
                            <input type="text" name="address" class="form-control" value="<?php echo $supplier['address']; ?>">
                        </div>
                        <div class="col-12">
                            <small class="text-muted">Essential supplier details only.</small>
                        </div>
                    </div>

                    <hr class="my-4">
                    <div class="d-flex gap-2 mb-3">
                        <button class="btn btn-primary btn-lg d-flex align-items-center justify-content-center gap-2 btn-icon"
                                type="submit" style="flex:2 1 0%;">
                            Save Supplier
                            <img src="/carriemart/assets/person-check-fill.svg" alt="" aria-hidden="true">
                        </button>
                        <button type="button"
                                class="btn btn-outline-secondary btn-lg d-inline-flex align-items-center justify-content-center gap-2 btn-icon-inverted"
                                style="flex:1 1 0%;" onclick="history.back()">
                            Go back
                            <img src="/carriemart/assets/caret-right-square.svg" alt="" aria-hidden="true">
                        </button>
                    </div>
                </form>

            </div>
        </div>
    </main>
</div>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-FKyoEForCGlyvwx9Hj09JcYn3nv7wiPVlz7YYwJrWVcXK/BmnVDxM+D2scQbITxI" crossorigin="anonymous"></script>
</body>
</html>



